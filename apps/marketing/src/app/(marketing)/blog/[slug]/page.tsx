...
export default async function BlogPostPage({ params }: BlogPostPageParams) {
      ...
      <AuroraPage variant="indigo" containerClassName="max-w-4xl space-y-12">
        <article className="space-y-10">
          <header className="space-y-5 text-center">
            <h1 className="text-3xl font-semibold text-[#0f172a] sm:text-4xl">記事が見つかりません</h1>
            <p className="text-sm text-neutral-600">
              お探しのブログ記事は現在公開されていないか、URL が変更されている可能性があります。
            </p>
          </header>
  ...
  const relatedPosts = allPosts
    ...
    .filter((p) => {
      ...
      return otherTags.some((tag: string) => tags.includes(tag));
    })
    .slice(0, 3);
  ...
  return (
    <AuroraPage variant="indigo" containerClassName="max-w-6xl space-y-12 pb-20 md:pb-0">
      <div className="grid gap-10 lg:grid-cols-[minmax(0,2.3fr)_minmax(260px,1fr)]">
        <article className="space-y-10">
            ...
            <nav className="flex flex-wrap items-center gap-1">
              ...
              {tags.length > 0 && (
                <>
                  ...
                  <span className="text-neutral-600">{tags[0]}</span>
                </>
              )}
            </nav>

            <BlogShareButtons shareUrl={shareUrl} shareText={post.title} variant="header" />
          </div>

          {/* カテゴリタブ風ナビゲーション */}
          <div className="flex flex-wrap gap-2 rounded-2xl bg-slate-50 px-4 py-2 text-xs">
            {[
              { href: "/blog", label: "ブログTOP" },
              { href: "/blog?tag=お知らせ", label: "お知らせ" },
              { href: "/blog?tag=活用ノウハウ", label: "活用ノウハウ" },
              { href: "/blog?tag=導入事例", label: "導入事例" },
            ].map((tab) => {
              const isActive = tags.includes(tab.label.replace("TOP", "").trim());
              return (
                <Link
                  key={tab.href}
                  href={tab.href}
                  className={
                    ...
                    (isActive
                      ...
                      : "bg-white text-neutral-600 hover:bg-primary/5 hover:text-primary")
                  }
                >
                  {tab.label}
                </Link>
          ...
          {coverImageUrl ? (
            ...
            <div className="overflow-hidden rounded-3xl bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 shadow-lg">
              <div className="aspect-[16/9] w-full px-8 py-7">
                ...
                <p className="mt-3 line-clamp-2 text-lg font-semibold text-white">{post.title}</p>
              </div>
            </div>
          )}
          ...
          <section className="mt-8 space-y-4 text-center">
            <p className="text-xs font-semibold tracking-[0.3em] text-neutral-500">＼ この記事をシェアする ／</p>
            <BlogShareButtons shareUrl={shareUrl} shareText={post.title} variant="footer" />
          </section>
          ...
          {relatedPosts.length > 0 && (
            <section className="mt-10 space-y-4">
              <h2 className="text-base font-semibold text-[#0f172a]">この記事も読まれています</h2>
        ...
        <div className="hidden space-y-6 lg:sticky lg:top-24 lg:bottom-24 lg:block">
          {latestPosts.length > 0 && (
            <section className="space-y-3">
              <h2 className="text-sm font-semibold text-[#0f172a]">最新の記事</h2>
      ...
      <div className="fixed inset-x-0 bottom-0 z-40 border-t border-slate-200/70 bg-white/95 px-4 py-3 shadow-[0_-4px_12px_rgba(15,23,42,0.08)] md:hidden">
        <div className="mx-auto flex max-w-3xl items-center justify-between gap-3">
          <div className="flex flex-col text-[11px] text-neutral-600">
            ...
            <span>店舗に合った活用方法を 30 分でご提案します。</span>
          </div>
          <Link
            href="/contact"
