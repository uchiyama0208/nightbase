...
export const TAG_OPTIONS = [
    ...
    { value: "help", label: "ヘルプ", color: "bg-orange-100 text-orange-700 dark:bg-orange-900 dark:text-orange-300", isStatus: true },
    { value: "ended", label: "終了", color: "bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-300", isStatus: true },
...
export function getCurrentTag(order: { cast_status?: string; item_name?: string }): TagValue {
    ...
    // cast_statusが7つのタグ値のいずれかであればそのまま返す
    if (status === 'waiting' || status === 'serving' || status === 'help' || status === 'ended' ||
