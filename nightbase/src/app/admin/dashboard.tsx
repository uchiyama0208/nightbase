...
async function fetchStatusCounts(supabase: any, table: string): Promise<StatusCounts> {
  ...
  if (publishedRes.error) {
    console.error(`${table}の公開件数取得に失敗しました`, publishedRes.error);
  }
  if (draftRes.error) {
    console.error(`${table}の下書き件数取得に失敗しました`, draftRes.error);
  }
    ...
    async function load() {
      ...
      try {
        const [blog, cases, manuals, latest] = await Promise.all([
          fetchStatusCounts(supabaseClient, "blog_posts"),
          fetchStatusCounts(supabaseClient, "case_studies"),
          fetchStatusCounts(supabaseClient, "manuals"),
      ...
      <section className="rounded-3xl border border-white/10 bg-white/5 p-6 shadow-[0_20px_60px_-20px_rgba(15,23,42,0.45)]">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-sm text-slate-400">Supabase に保存された最新の更新履歴を確認できます。</p>
          </div>
        </div>
