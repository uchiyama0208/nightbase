  ...
  return (
    <AuroraPage variant="teal" containerClassName="max-w-5xl space-y-16">
      <header className="glass-panel space-y-5 p-10 text-center">
        <div className="flex flex-wrap items-center justify-center gap-3 text-xs font-semibold uppercase tracking-[0.3em] text-neutral-400">
          <span className="inline-flex items-center rounded-full border border-primary/30 bg-primary/10 px-4 py-1 text-[10px] text-primary">
            {industryLabel}
          </span>
          {publishedLabel && <span>{publishedLabel}</span>}
        </div>
        <div className="space-y-3">
          <h1 className="text-4xl font-semibold text-[#0f172a] sm:text-5xl">{caseStudy.title}</h1>
      ...
      <section className="space-y-10">
        <div className="glass-panel grid gap-8 p-8 lg:grid-cols-3">
          <div className="space-y-3 lg:col-span-1">
            <h2 className="text-sm font-semibold uppercase tracking-[0.3em] text-primary">導入前の課題</h2>
            <p className="text-sm text-neutral-600">現場で直面していた課題を整理し、NightBaseで解決すべきポイントを明確にしました。</p>
          </div>
        ...
        <div className="glass-panel grid gap-8 p-8 lg:grid-cols-3">
          <div className="space-y-3 lg:col-span-1">
            <h2 className="text-sm font-semibold uppercase tracking-[0.3em] text-primary">導入後の変化</h2>
            <p className="text-sm text-neutral-600">導入後に得られた成果やお客様の声をピックアップしています。</p>
          </div>
          <div className="lg:col-span-2">
            <ul className="space-y-3 text-sm text-neutral-700">
              {results.length > 0 ? (
      ...
      <section className="glass-panel space-y-6 p-8 text-center">
        <h2 className="text-2xl font-semibold text-[#0f172a]">NightBase の導入についてご相談ください</h2>
        ...
        <div className="flex flex-wrap items-center justify-center gap-4">
          ...
          <Link href="/industries" className="glass-button">
            夜職別の活用を見る
          </Link>
        </div>
      </section>
