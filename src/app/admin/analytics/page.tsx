...
const TABLE_CONFIG = [
  { key: "blog", table: "blog_posts", label: "ブログ", slugField: "slug", contentField: "content" },
  {
    key: "case",
    table: "case_studies",
    label: "導入事例",
  ...
  { key: "manual", table: "manuals", label: "マニュアル", slugField: "slug", contentField: "body_markdown" },
] as const;
        ...
        TABLE_CONFIG.map(async (config) => {
          ...
          return rows.map((row) => {
            ...
            if (isCaseStudyConfig) {
              ...
              contentValue = typeof row?.[config.contentField] === "string" ? row[config.contentField] : null;
            }
