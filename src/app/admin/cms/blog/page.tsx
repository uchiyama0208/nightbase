...
function BlogListContent({ supabase }: { supabase: any }) {
  ...
  const loadPosts = useCallback(async () => {
    ...
    const { data, error } = await supabase
      .from("cms_entries")
      .select("id, type, title, slug, status, tags, published_at, cover_image_url")
      .eq("type", "blog")
    ...
    return (
      <div className="p-8">
        <div className="space-y-4 rounded-2xl border border-red-500/40 bg-red-500/10 p-8 text-center text-red-900">
          <p>{state.error}</p>
          <Button variant="outline" className="border-red-300 text-red-900" onClick={loadPosts}>
            再読み込み
          </Button>
        </div>
      </div>
  ...
  return (
    <div className="space-y-10 p-8">
      <CmsListLayout
        title="ブログ記事"
        description="ブログ記事の公開・下書きを管理します。"
