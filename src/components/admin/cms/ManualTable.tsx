...
export function ManualTable({ items, categories }: ManualTableProps) {
  const [search, setSearch] = useState("");
  const [status, setStatus] = useState<(typeof STATUS_TABS)[number]["value"]>("all");
  const [category, setCategory] = useState<string>("すべて");

  const options = useMemo(() => ["すべて", ...categories], [categories]);
  ...
  const filteredItems = useMemo(() => {
    return items.filter((item) => {
      ...
      const matchesCategory = category === "すべて" ? true : item.category === category;
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex flex-col gap-2">
          ...
          <p className="text-sm text-slate-400">manual_pages テーブルの公開・下書きマニュアルを編集します。</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-60">
              ...
              <DropdownMenuSeparator />
              {options.map((option) => (
                <DropdownMenuItem key={`manual-dropdown-${option}`} onSelect={() => setCategory(option)}>
                  {option}
                </DropdownMenuItem>
              ))}
            </DropdownMenuContent>
            ...
            {filteredItems.length === 0 ? (
              ...
              filteredItems.map((item) => (
                <TableRow key={item.id} className="border-white/5">
                  ...
                  <TableCell className="text-sm text-slate-300">{item.category}</TableCell>
