  ...
  const filteredItems = useMemo(() => {
    return items.filter((item) => {
      const matchesSearch = search
        ? `${item.title} ${item.company_name}`.toLowerCase().includes(search.toLowerCase())
        : true;
      const matchesStatus =
            ...
            {filteredItems.length === 0 ? (
              ...
              filteredItems.map((item) => (
                <TableRow key={item.id} className="border-white/5">
                  ...
                  <TableCell className="text-sm text-slate-300">{formatCaseStudyIndustry(item.industry)}</TableCell>
                  <TableCell>
                    <Badge
